...
INPUT_DIR = "example_CVs/"
OUTPUT_DIR = "../data/input_CVs/"
NUM_SYNTHETIC_CVS = 10  # Number of synthetic CVs you want to generate
...
def run_pipeline():
    ...
    for j in range(9, len(examples)-1):
        for i in range(16, 11 + NUM_SYNTHETIC_CVS):
            synthetic_cv = generate_synthetic_cv(examples[j])
            if synthetic_cv:
               save_cv(synthetic_cv, j + 1, i)
               print(f"Saved: synthetic_cv_{j}_{i}.txt")
